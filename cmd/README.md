...
- `platforms.yaml`：平台配置（mapping / filter / ui_layer）
- `platforms.sh`：薄封装（用 yq 读取 `platforms.yaml`，对外提供 bash 函数）
- `init.sh`：首次初始化（把 `cmd/<ui>/{installation.yaml,user.yaml}` 以 rsync 方式写入目标目录，且只写入缺失文件）
- `update.sh`：日常更新
...
- 默认使用 `--delete` 清理合并结果中不存在的文件；如需保留目标端多余文件，可用 `--no-delete` 或 `RIME_RSYNC_DELETE=0`。
