...
local extraParams = {
  insets: params.keyboard.T9button.backgroundInsets.iPhone.landscape,
};

{
  new(isDark, isPortrait):
    local insets = if isPortrait then params.keyboard.T9button.backgroundInsets.iPhone.portrait else params.keyboard.T9button.backgroundInsets.iPhone.landscape;
    ...
    + basicStyle.newT9ButtonBackgroundStyle(isDark, extraParams)
    ...
    + basicStyle.newCommitCandidateForegroundStyle(isDark, { systemImageName: 'minus',fontWeight: 'medium', fontSize: 19})
		...
		{"spaceButtonForegroundStyle2" :
       {
        ...
        "center": { "x": 0.13 , "y":0.25 },
        "fontSize": 10,
        "highlightColor": if isDark then colors.labelColor.secondary.dark else colors.labelColor.secondary.light, 
        "normalColor" : if isDark then colors.labelColor.secondary.dark else colors.labelColor.secondary.light, 
        "text": "$rimeSchemaName"
        }
				}
    // Notifications
